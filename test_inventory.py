...
class TestInventoryItem:
    ...
    def test_is_low_stock_true_when_at_threshold(self):
        item = InventoryItem(
            sku="SKU1", name="Widget", quantity=5, low_stock_threshold=5
        )
        assert item.is_low_stock() is True

    def test_is_low_stock_false_when_above_threshold(self):
        item = InventoryItem(
            sku="SKU1", name="Widget", quantity=6, low_stock_threshold=5
        )
...
class TestInventoryItemManagement:
    ...
    def test_add_duplicate_sku_raises_value_error(self, inventory, widget):
        ...
        duplicate = InventoryItem(
            sku="WIDGET1", name="Widget Copy", quantity=5
        )
...
class TestStockOperations:
    ...
    def test_restock_zero_or_negative_raises_value_error(
        self, stocked_inventory, bad_amount
    ):
        with pytest.raises(ValueError):
    ...
    def test_sell_exact_remaining_quantity_results_in_zero(
        self, stocked_inventory
    ):
        new_quantity = stocked_inventory.sell("WIDGET1", 10)
        assert new_quantity == 0

    def test_sell_more_than_available_raises_out_of_stock(
        self, stocked_inventory
    ):
    ...
    def test_sell_zero_or_negative_raises_value_error(
        self, stocked_inventory, bad_amount
    ):
        with pytest.raises(ValueError):
            stocked_inventory.sell("WIDGET1", bad_amount)

    def test_sell_does_not_change_quantity_when_it_fails(
        self, stocked_inventory, widget
    ):
...
class TestReporting:
    ...
    def test_total_inventory_value(self):
        assert self.inventory.total_inventory_value() == 20
    ...
    def test_low_stock_items_returns_only_low_stock(self):
        low_stock_skus = {
            item.sku for item in self.inventory.low_stock_items()
        }
    ...
    def test_low_stock_items_empty_when_all_well_stocked(self):
        ...
        inv.add_item(
            InventoryItem(
                sku="A", name="Item A", quantity=100, low_stock_threshold=5
            )
        )
...
class TestNotificationsAndClock:
    ...
    def test_transaction_log_records_multiple_operations(
        self, notified_inventory
    ):
        ...
        actions = [
            entry["action"]
            for entry in notified_inventory.get_transaction_log()
        ]
